...
<?php
$sql = "SELECT * FROM `tb_order` WHERE `status` = 2";
$result = $conn->query($sql);
$result->num_rows;
?>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                <tr>
                    ...
                    <td><a class="btn btn-primary" href="<?php echo SITE_URL; ?>kitchen/view_kot.php?order_id=<?php echo $row['order_id']; ?> && status=<?php echo $row['status']; ?>"><i class="fas fa-eye"></i></a></td>
                </tr>
                <?php
            }
        }
