...
if (isset($_GET['order_id'])) {
    $getorder_id = clean_data($_GET['order_id']);

    $sql = "UPDATE `tb_order` SET `status`=6 WHERE `order_id`=$getorder_id";
    $getorder = $conn->query($sql);
}
?>
<?php
$osql = "SELECT * FROM `tb_order` WHERE `order_id`=$getorder_id";
$oresult = $conn->query($osql);
$orow = $oresult->fetch_assoc();
?>
<div class="row">
    <a type="button" class="btn btn-outline-danger btn-sm" href="on_delivery.php"><i class="fas fa-hand-point-left"></i></a>
...
<div class="row order-card">
        ...
        <div class="card-body">
            ...
            <div class="card-body"> 
                ...
                <strong>
                    <div class="row">
                        <?php
                        $sql_Dpn = "SELECT * FROM `tb_order` o LEFT JOIN tb_employee e ON o.delivery_person = e.user_id WHERE o.order_id = $getorder_id";
    ...
    <div class="card" style="width: 47rem;">
        <div class="card-body">
            ...
            <?php
            $sql = "SELECT * FROM `tb_order_item` i LEFT JOIN tb_menu m ON m.item_id=i.item_id WHERE i.order_id=$getorder_id";
